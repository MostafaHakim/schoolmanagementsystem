// src/components/AllExamResults.jsx
...
const AllExamResults = () => {
  ...
  const getStudentOverallPerformance = (student) => {
    ...
    const overallGrade = hasAnyFGrade ? "F" : getSimpleGrade(overallPercentage);
    const passRate =
      totalSubjects > 0
  ...
  // সব ছাত্রের প্রিন্ট (ব্যাচ প্রিন্ট)
  const printAllStudents = async () => {
    ...
    if (
      !window.confirm(
        `Print ${students.length} students' reports? This will open multiple print dialogs.`
      )
    ) {
      return;
    }
    ...
    const printSequentially = async (index) => {
      ...
      const content = generateMarkSheetContent(student);
      ...
      setTimeout(() => {
        ...
        setTimeout(() => printSequentially(index + 1), 1500);
      }, 500);
    };

    await printSequentially(0);
  };

  // মার্কশিট কন্টেন্ট জেনারেটর (আপনার চাহিদা অনুযায়ী)
  const generateMarkSheetContent = (student) => {
    const overallPerformance = getStudentOverallPerformance(student);

    // প্রথম ৪টি এক্সাম নেওয়া (১ম সেমিস্টার, অর্ধবার্ষিক, ২য় সেমিস্টার, বার্ষিক)
    ...
    return `
      ...
      <html>
      <head>
        <title>${student.className} - Mark Sheet</title>
        <style>
          @page {
            size: A4 landscape;
            margin: 5mm;
      ...
      <body>
        <div class="marksheet-container">
          <div class="header">
            ...
            <div class="report-title">ACADEMIC MARK SHEET</div>
            <div style="font-size: 14px; color: #555;">
              Academic Year: ${student.studentSessions || "2024"} • Class: ${
      student.studentClass
          ...
          <div style="margin-top: 2px; padding: 15px; background: ${
            ...
            <div style="display: flex; justify-content: space-between; font-size: 11px;">
              ...
              <span>Total Marks: ${overallPerformance.totalMarksAllExams}/${
                  overallPerformance.totalPossibleAllExams
                }</span>
            </div>
          </div>
          `
          ...
          <!-- Remarks -->
          <div style="margin-top: 5px; padding: 5px; border: 1px dashed #999;">
        ...
        <div className="fixed top-4 right-4 bg-white shadow-2xl rounded-xl p-4 z-50 min-w-64">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div
              ...
              style={{
                width: `${(currentPrintIndex / students.length) * 100}%`,
              }}
            ></div>
          </div>
          <div className="text-right mt-1 text-sm text-gray-600">
            {currentPrintIndex} of {students.length} students
          </div>
          <p className="text-xs text-gray-500 mt-2">
            Each report fits in one A4 page (Landscape)
          </p>
        </div>
      )}
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            ...
            <div className="flex flex-col md:flex-row gap-3">
              ...
              <button
                ...
                className="px-6 py-3 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-xl font-bold hover:from-purple-600 hover:to-purple-700 transition-all shadow-xl hover:shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {printingAll ? (
                  <>
                    <div className="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full"></div>
                    Printing... ({currentPrintIndex}/{students.length})
                  </>
                ) : (
          ...
          {selectedStudent && (
            <div
              dangerouslySetInnerHTML={{
                __html: generateMarkSheetContent(selectedStudent),
              }}
            />
          )}
        </div>
